...
public class SkeletonMeshSlicer : MonoBehaviour
{
    ...
    public class SliceTarget
    {
        ...
        public List<Transform> sectionsBones,
            posNoneSectionBones,
            negNoneSectionBones,
            posInvisibleBones,
            negInvisibleBones,
            allRootBones = new List<Transform>();
        ...
        public class SliceThread
        {
            ...
            public void SliceMesh(object obj)
            {
                ...
                sliceTarget.posGoesOld =
                    sliceTarget._sMDataBase.posPoints.Count
                    > sliceTarget._sMDataBase.negPoints.Count;
                var points = sliceTarget.posGoesOld
                    ? sliceTarget._sMDataBase.negPoints
                    : sliceTarget._sMDataBase.posPoints;
                sliceTarget.SeparateVerticesWeights(points);
                sliceTarget.GetAllInvisableBones(
                    sliceTarget._sMDataBase.posPoints,
                    sliceTarget.posInvisibleBones,
                    true
                );
                sliceTarget.GetAllInvisableBones(
                    sliceTarget._sMDataBase.negPoints,
                    sliceTarget.negInvisibleBones,
                    false
                );
                sliceFinish = true;
            }
        ...
        public class SliceColliderHelper : SliceThread
        {
            ...
            {
                var otherSideSectionBone = sliceTarget._sMDataBase.Bones[
                    sliceTarget.mapedBones[sectionBone]
                ];
                ...
                {
                    var colMeshData = colMeshDatas[i];
                    Mesh posMesh = colMeshData.GeneratePartialMesh(
                        colMeshData.posPoints,
                        colMeshData.posTriangles
                    );
                    Mesh negMesh = colMeshData.GeneratePartialMesh(
                        colMeshData.negPoints,
                        colMeshData.negTriangles
                    );
                    ...
                    if (
                        boundsLength.x > minBoundsLength
                        && boundsLength.y > minBoundsLength
                        && boundsLength.z > minBoundsLength
                    )
        ...
        public SliceTarget(SkeletonMeshSlicer sMSlicer, SkinnedMeshRenderer sMRender, int i)
        {
            _sMSlicer = sMSlicer;
            _sMRender = sMRender;
            _sMDataBase = new SmMeshDatas(
                _sMRender.sharedMesh,
                _sMRender.rootBone.parent,
                _sMRender.sharedMaterials,
                _sMRender.bones
            );
            targetIndex = i;
        ...
        void AddBoneToList(
            bool isPos,
            List<Transform> posBoneList,
            List<Transform> negBoneList,
            Transform bone
        )
        {
            if (isPos)
        ...
        void DistinguishBothSideIndexes(SmMeshDatas nowDataBase)
        {
            ...
            {
                bool side = _sMSlicer._slicePlane.GetSide(
                    nowDataBase.VerticesWorldPos[verticesIndex]
                );
            ...
            if (nowDataBase.posPoints.Count > 0 && nowDataBase.negPoints.Count > 0)
            {
                ...
                for (
                    int subMeshIndex = 0;
                    subMeshIndex < nowDataBase.SubMeshes.Count;
                    ++subMeshIndex
                )
                {
                    ...
                    for (
                        int trangleIndex = subTriangles.Count - 3;
                        trangleIndex >= 0;
                        trangleIndex -= 3
                    )
                    {
                ...
                AddSectionTrianglesInMesh(
                    posSectionPoints,
                    nowDataBase.sectionPosTriangles,
                    nowDataBase.posTriangles,
                    nowDataBase
                );
                AddSectionTrianglesInMesh(
                    negSectionPoints,
                    nowDataBase.sectionNegTriangles,
                    nowDataBase.negTriangles,
                    nowDataBase
                );
                ...
                foreach (var iV in nowDataBase.slicedSectionEdgeIndexes)
                    sectionEdges.Add(
                        new IndexVector(posSectionPoints[iV.From], posSectionPoints[iV.To])
                    );
                ...
                if (_sMSlicer.nowCapMode != CapMode.FORCE_NOT_ADD)
                {
                    ...
                    List<List<int>> posOrderedCapEdges = new List<List<int>>(),
                        posCapsTriangles = new List<List<int>>(),
                        negOrderedCapEdges = new List<List<int>>(),
                        negCapsTriangles = new List<List<int>>();
                    DivideDifferentEdge(sectionEdges, posOrderedCapEdges, nowDataBase);
                    posOrderedCapEdges = nowDataBase
                        .DuplicateVerticesData(posOrderedCapEdges)
                        .Item1;
                    List<List<Vector2>> capsUVList = CreateAllCaps(
                        posOrderedCapEdges,
                        posCapsTriangles,
                        negCapsTriangles,
                        nowDataBase
                    ...
                    List<Vector3> posCapNormal = new List<Vector3>(),
                        negCapNormal = new List<Vector3>();
                    CalculateCapNormalsAndSwitchCapSide(
                        posCapsTriangles,
                        negCapsTriangles,
                        posCapNormal,
                        negCapNormal,
                    ...
                    nowDataBase.SetVerticesNormalsAndUVs(
                        posOrderedCapEdges,
                        posCapNormal,
                        capsUVList
                    );
        ...
        void AddSectionTrianglesInMesh(
            List<int> nowSectionPoints,
            List<List<SectionTriangle>> nowDiffSectionTriangles,
            List<int> nowSideTriangles,
            SmMeshDatas nowDataBase
        )
        {
        ...
        List<List<Vector2>> CreateAllCaps(
            List<List<int>> posOrderedCapEdges,
            List<List<int>> posCapsTriangles,
            List<List<int>> negCapsTriangles,
            SmMeshDatas nowDataBase
        )
        {
            List<List<Vector2>> capsUVList = new List<List<Vector2>>(),
                capsVertices2D = new List<List<Vector2>>();
            foreach (var edge in posOrderedCapEdges)
            {
                var capVertices2D = ConvertModalV3ToPlaneV2(edge, nowDataBase);
                var newTriangles1 = FillCapByTriangles(
                    new List<int>(edge),
                    new List<Vector2>(capVertices2D),
                    true
                );
                var newTriangles2 = FillCapByTriangles(
                    new List<int>(edge),
                    new List<Vector2>(capVertices2D),
                    false
                );
                ...
                var capTriangles =
                    newTriangles1.Count > newTriangles2.Count ? newTriangles1 : newTriangles2;
        ...
        void DivideDifferentEdge(
            List<IndexVector> sectionEdges,
            List<List<int>> orderedCapEdges,
            SmMeshDatas nowDataBase
        )
        {
            ...
            while (sectionEdges.Count > 0)
            {
                ...
                while (i < sectionEdges.Count)
                {
                    if (
                        Mathf.Approximately(
                            (
                                nowDataBase.Vertices[nowEdge.From]
                                - nowDataBase.Vertices[sectionEdges[i].From]
                            ).magnitude,
                            0f
                        )
                    )
                    {
                        ...
                        Mathf.Approximately(
                            (
                                nowDataBase.Vertices[nowEdge.To]
                                - nowDataBase.Vertices[sectionEdges[i].To]
                            ).magnitude,
                            0f
                        )
                    )
                ...
                if (
                    ...
                    && (
                        ...
                        || _sMSlicer.nowCapMode == CapMode.FORCE_ADD
                    )
                )
        ...
        void CalculateCapNormalsAndSwitchCapSide(
            List<List<int>> posCapsTriangles,
            List<List<int>> negCapsTriangles,
            List<Vector3> posCapNormals,
            List<Vector3> negCapNormals,
            List<List<int>> posOrderedCapEdges,
            List<List<int>> negOrderedCapEdges,
            SmMeshDatas nowDataBase
        )
        {
            for (int iC = 0; iC < posCapsTriangles.Count; iC++)
            {
                ...
                if (triangles.Count > 2)
                {
                    ...
                    if (
                        Vector3.Dot(
                            Vector3.Cross(dir1, dir2).normalized,
                            _sMSlicer._slicePlane.normal
                        ) > 0f
                    )
                    {
        ...
        void GetAllInvisableBones(
            List<int> sideVertices,
            List<Transform> invisibleBones,
            bool isPos
        )
        {
            ...
            var sideBonesIndexRange =
                isPos == posGoesOld
                    ? new int[2] { 0, boneSliceIndex }
                    : new int[2] { boneSliceIndex, _sMDataBase.Bones.Count };
    ...
    public class SliceableTriangle
    {
        ...
        /// </summary>
        static void CalculateWithOneNegTriangle(
            int neg,
            int posA,
            int posB,
            SmMeshDatas nowDataBase,
        ...
        )
        {
            ...
            nowDataBase.sectionNegTriangles[subMeshIndex].Add(
                new SectionTriangle(neg, iA, iB, new int[2] { 1, 2 }, new int[1] { 0 })
            );
            nowDataBase.sectionPosTriangles[subMeshIndex].Add(
                new SectionTriangle(posA, iB, iA, new int[2] { 1, 2 }, new int[1] { 0 })
            );
            nowDataBase.sectionPosTriangles[subMeshIndex].Add(
                new SectionTriangle(posA, posB, iB, new int[1] { 2 }, new int[2] { 0, 1 })
            );
        ...
        /// </summary>
        static void CalculateWithTwoNegTriangle(
            int pos,
            int negA,
            int negB,
            SmMeshDatas nowDataBase,
        ...
        )
        {
            ...
            nowDataBase.sectionNegTriangles[subMeshIndex].Add(
                new SectionTriangle(negA, negB, iB, new int[1] { 2 }, new int[2] { 0, 1 })
            );
            nowDataBase.sectionNegTriangles[subMeshIndex].Add(
                new SectionTriangle(negA, iB, iA, new int[2] { 1, 2 }, new int[1] { 0 })
            );
            nowDataBase.sectionPosTriangles[subMeshIndex].Add(
                new SectionTriangle(pos, iA, iB, new int[2] { 1, 2 }, new int[1] { 0 })
            );
        ...
        public SliceableTriangle FindEdgeSharedTriangle(List<SliceableTriangle> slicedTrangles)
        {
            ...
            {
                int pointEqualNum =
                    Convert.ToInt32(IsSharePoint(newTriangle.i1))
    ...
    public void SliceByMeshPlane(
        Vector3 planeNormal,
        Vector3 planePosition,
        Action whenSliceFinish = null
    )
    {
