...
public class SmMeshDatas
{
    ...
    {
        get { return Materials != null; }
    }

    public List<int> posPoints,
        negPoints,
        posTriangles,
        negTriangles;
    ...
    /// </summary>
    public List<List<SectionTriangle>> sectionPosTriangles,
        sectionNegTriangles;
    ...
    public void AddNewPoint(int from, int to, float ratioIn)
    {
        ...
        if (BoneWeightsExists)
        {
            ...
            var wsArr = ws.Where(v => v.Value != 0)
                .Take(4)
    ...
    public void AddNewBones(
        Transform[] oldBones,
        Transform[] newBones,
        Dictionary<Transform, int> mapedBones
    )
    {
    ...
    public void SetVerticesNormalsAndUVs(
        List<List<int>> verticeIndexes,
        List<Vector3> newNormals,
        List<List<Vector2>> capsUVList
    )
    {
        for (int iV = 0; iV < verticeIndexes.Count; iV++)
        {
    ...
    public Vector3 GetVerticeWorldPos(int index)
    {
        ...
        if (BoneWeightsExists)
        {
            ...
            if (boneWeight.weight0 > 0f)
                pointWorldPosition +=
                    PointsLtoW[boneWeight.boneIndex0].MultiplyPoint3x4(position)
                    * boneWeight.weight0;
            if (boneWeight.weight1 > 0f)
                pointWorldPosition +=
                    PointsLtoW[boneWeight.boneIndex1].MultiplyPoint3x4(position)
                    * boneWeight.weight1;
            if (boneWeight.weight2 > 0f)
                pointWorldPosition +=
                    PointsLtoW[boneWeight.boneIndex2].MultiplyPoint3x4(position)
                    * boneWeight.weight2;
            if (boneWeight.weight3 > 0f)
                pointWorldPosition +=
                    PointsLtoW[boneWeight.boneIndex3].MultiplyPoint3x4(position)
                    * boneWeight.weight3;
        }
        else
    ...
    public void SetLtoWMatrix()
    {
        ...
        if (BoneWeightsExists)
        {
            ...
            for (int i = 0; i < Bones.Count; i++)
            {
                ...
                PointsLtoW.Add(
                    bone != null ? bone.localToWorldMatrix * Bindposes[i] : new Matrix4x4()
                );
            }
        }
        else
